Given /^a "(.*)" subscriber$/ do |plan|
  reate_subscriber( :username => 'subscriber', :subscription_plan => SubscriptionPlan.find_by_name(plan) )
end

Given /^a "(.*)" subscriber is logged in$/ do |plan|
  create_subscriber( :username => 'subscriber', :subscription_plan => SubscriptionPlan.find_by_name(plan) )
  log_in_subscriber
end
...
Given /^a "(.*)" subscriber who is (in trial|active|past due|expired), with next renewal (.*), and profile (has no info|is authorized|has an error)$/ do |plan, subs_state, date_text, profile_state |
  ...
  subscriber = create_subscriber( :username => 'subscriber', :subscription_plan => SubscriptionPlan.find_by_name(plan) )
